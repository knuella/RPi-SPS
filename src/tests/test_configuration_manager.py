...
class HandleRequest(unittest.TestCase):
    ...
    def setUp(self, mock_context):
        self.config = ConfigurationManager()
        self.context_mock = self.config.context
    ...
    @mock.patch.object(ConfigurationManager, "handle_request_value")
    def test_reply_error_request_value_message_error(self, request_value_mock):
        """'reply_error' is called with the correct arguments when a
        MessageFormatError occurs in handle_request_value"""
